 ...
 */
public
class MembershipForm  implements Viewable{
    private final
...
/*---------------------------------------------------------------*/
        ...
        vBox.setSpacing(10);
        vBox.getChildren().addAll(labelHeader,hBox,textName,textFatherName, hBox2,hBox1,textAreaAddress,hBox3,btnSubmit,labelMessage);
    }
...
//Inserting Details to Table
                    try {
                        ...
                        if (EmpDetailsToDB.insertMemberDetailToTable(membershipFormCollection, objGtpConstant,
                                                                     objDBAction) > 0) {
                            System.out.println("Data Successfully inserted to membership Form Table");
                            labelMessage.setText(String.format("Data with MembeshipID:- %s & Name :- %s. Added successfully",textMembershipNum.getText()
                                                              ,textName.getText()));
                            labelMessage.setTextFill(Color.GREEN);

                            GreentechConstants.Global_Logger.log(Level.INFO,
                                                                 "Data Successfully inserted to membership Form Table"+
                                                                 String.format("Data with MembeshipID:- %s & Name :- %s. Added successfully",textMembershipNum.getText()
                                                                 ,textName.getText()));
                        }else{
                            labelMessage.setText(String.format("Data with MembeshipID:- %s & Name :- %s. Add Failed",textMembershipNum.getText()
                                    ,textName.getText()));

                            GreentechConstants.Global_Logger.log(Level.SEVERE,
                                                                 String.format("Data with MembeshipID:- %s & Name :- %s. Add Failed",textMembershipNum.getText()
                                                                         ,textName.getText()));
                        }
